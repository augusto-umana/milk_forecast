---
title: "Modelo Predictivo Acopio Formal Leche. Variable Objetivo Litros promedio por día"
...
```{r}
acopio_formal %>% 
  select(fecha, starts_with("litros")) %>% 
  pivot_longer(-fecha) %>% 
  ggplot(aes(x=fecha, y = value))+
  geom_line()+
  geom_point()+
  scale_x_date(name = "Fecha", date_breaks = "1 year",date_labels = "%Y")+
  scale_y_continuous(name = "Millones de litros", labels = scales::label_comma(scale = 1/1000000))+
...
split_2010_2022 %>% 
  ...
  plot_time_series_cv_plan(.date_var = fecha, .value = litros_diarios_acopio_formal,.interactive = FALSE )
```
...
```{r}
mat_correlacion <- 
  cor(x = datos_receta$litros_diarios_acopio_formal,
    y=datos_receta %>% select(-fecha, -litros_diarios_acopio_formal) %>% as.matrix(),use = "pair") %>% 
...
grafica_variables_correlacionadas <- variables_correlacionadas %>% 
  ...
  ggtitle("Variables Correlacionadas con Acopio Formal de Leche (promedio por día)")+
  theme_bw()+
...
saveRDS(object = modelo_optimo, 
  ...
  config$modelos_pronostico$modelo_promedio_diario))
```
